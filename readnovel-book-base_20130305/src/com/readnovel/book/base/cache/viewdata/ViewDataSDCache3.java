...
public class ViewDataSDCache3 extends AbsViewDataSDCache<JSONArray> {
	private static volatile ViewDataSDCache3 instance;
	private final String rootPath;

	private ViewDataSDCache3(String path) {
		//初始化根路径
	 ...
	 * @return
	 */
	public static ViewDataSDCache3 getInstance(String path) {
		if (instance == null) {
			synchronized (ViewDataSDCache3.class) {
				if (instance == null) {
					instance = new ViewDataSDCache3(path);
				}
			}
		}
		return instance;
	...
	// 从缓存中取
	@Override
	public JSONArray get(String url, KeyCreater keyCreater) {

		return get(url, keyCreater, StorageUtils.VIEW_DATA_TIME_OUT);
	}

	@Override
	public JSONArray get(String url, KeyCreater keyCreater, long timeOut) {
		if (!available())
			return null;// 不可用直接返回
			...
			if (time != 0 && curTime - time > timeOut) {//超过缓存时间删除
				...
				try {
					if (StringUtils.isNotBlank(viewData))
						jo = new JSONArray(viewData);//将缓存数据组织成JSONObject
	...
	// 放入缓存
	@Override
	public boolean put(String url, JSONArray viewData) {
	...
	// 放入缓存,根据一定条件
	@Override
	public boolean put(String url, JSONArray viewData, Filter<JSONArray> filter) {

		return put(url, viewData, filter, null);
	}

	@Override
	public boolean put(String url, JSONArray viewData, Filter<JSONArray> filter, KeyCreater keyCreater) {
		if (!available())
