				...
				case AlixId.RQF_PAY: {
					...
					try {
						...
						if (retVal == ResultChecker.RESULT_CHECK_SIGN_FAILED) {// 验签失败
							BaseHelper.showDialog(act, "提示", act.getResources().getString(R.string.check_sign_failed),
									android.R.drawable.ic_dialog_alert);
						} else {//判断交易码
							if (tradeStatus.equals(PAY_SUCCESS))//判断交易状态码，只有9000表示交易成功
								ViewUtils.showDialog(act, "订单支付成功", R.drawable.infoicon, positiveListener);
							else
								ViewUtils.showDialog(act, "订单支付失败", R.drawable.infoicon, null);

						}

					} catch (Exception e) {
						//						BaseHelper.showDialog(act, "提示", strRet, R.drawable.infoicon);
